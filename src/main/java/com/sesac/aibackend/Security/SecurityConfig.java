    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                                ...
                                .requestMatchers(
                                        "/login", "/signup", "/health",
                                        "/oauth2/**", "/login/oauth2/**", //** 이기호는 뭐든 포함
                                        "/swagger-ui.html", "/swagger-ui/**", "/v3/api-docs/**",
                                        "/h2-console/**", "/error"
                                ).permitAll() //>다 허용해. 뭐든 들어오든!
                                // Day 2 인메모리 CRUD 학습용 (운영 권장 X)
                                .requestMatchers("/legacy/items/**").permitAll()
                                // 관리자 전용
                                .requestMatchers("/admin/**").hasRole("ADMIN") //hasrole
                                // 그 외 모두 인증 필요 (Day 3 JPA CRUD, /chat 등)
                                .anyRequest().authenticated()
                        //>앤드포인트외 모든 것들은 인정처리를 맞춰야한다!(anytequest의 뜻)
                )
